...
function writeResultToFile($result) {
    ...
    $file_name = "php_test_result_dump.txt";
    
    file_put_contents($file_name,json_encode($result).PHP_EOL, FILE_APPEND);
}
...
function test_benchmark($settings)
{
    ...
    test_math($result); //$result['benchmark']['math']
    test_string($result); ///$result['benchmark']['string']
    test_loops($result);  //$result['benchmark']['loops']
    test_ifelse($result); //$result['benchmark']['ifelse']
